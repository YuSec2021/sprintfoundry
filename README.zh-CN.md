...
- 进度推进依赖文件产物，而不是聊天记忆。
- sprint 完成的唯一权威信号是 `.sprintfoundry/results/eval/eval-result-{N}.md` 包含 `SPRINT PASS`。
...
| `.sprintfoundry/prompts/sprint-{N}-{action}.md` | Orchestrator | 当前 contract、implementation 或 retry 交接的完整 Codex prompt；Codex CLI 命令行只接收读取该文件的短指令 |
...
| `.sprintfoundry/results/eval/eval-result-{N}.md` | Evaluator | sprint 结论和证据；只有 `SPRINT PASS` 才代表完成 |
| `.sprintfoundry/state/run-state.json` | Orchestrator | 当前模式、重试计数、活跃分支、暂停状态、版本元数据 |
