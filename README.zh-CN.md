...
| 角色 | 运行时 | 职责 |
| --- | --- | --- |
| Planner | Claude sub-agent | 将短需求转成产品方向、验证模式和 sprint 计划 |
| Generator | Codex CLI | 实现一个已批准 sprint，自检、提交，并写入 `eval-trigger.txt` |
| Evaluator | Claude sub-agent + 验证工具 | 审核 contract，并通过配置的外部表面验证已提交工作 |
| Orchestrator | `sprintfoundry-orchestrator` skill | 读取文件状态、选择下一步路由、调用代理，并在不安全状态下暂停 |
...
- Codex 不评估自己的输出。
- 进度推进依赖文件产物，而不是聊天记忆。
- sprint 完成的唯一权威信号是 `eval-result-{N}.md` 包含 `SPRINT PASS`。
...
flowchart TD
    ...
    P -- 否 --> PL["Planner 创建 spec、init.sh、progress log"]
    ...
    G --> T["Codex 提交并写 eval-trigger.txt"]
...
| `eval-trigger.txt` | Generator | 表示已提交 sprint 等待 quality gate 和评估 |
| `quality-gate-{N}.md` | Orchestrator | Evaluator CHECK 前的静态质量门禁结果 |
| `eval-result-{N}.md` | Evaluator | sprint 结论和证据；只有 `SPRINT PASS` 才代表完成 |
...
`run-state.json`、`eval-result-*.md`、`quality-gate-*.md`、`VERSION`、`CHANGELOG.md` 等运行态文件在本仓库中被忽略，因为它们属于消费 plugin 的目标项目。
