...
- **多层防御**：路由前审计（set-based 进度：唯一阻塞项是 run-state 谎报无 eval-result 支持的 PASS；乱序低序号 sprint 视为 pending 而非违规）、sprint-fence 边界检查、pre-commit 钩子、append-only 审计日志（`harness-audit.ndjson`）。
