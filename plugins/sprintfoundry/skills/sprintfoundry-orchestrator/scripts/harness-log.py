...
The authoritative timeline lives in `.sprintfoundry/logs/harness-audit.ndjson`. This tool is a
...
EVAL_RESULTS_DIR = ".sprintfoundry/results/eval"
RUN_STATE_FILE = ".sprintfoundry/state/run-state.json"
...
def read_records(project_dir: Path) -> list[dict[str, Any]]:
    path = project_dir / AUDIT_FILE
    if not path.exists():
        path = project_dir / ".sprintfoundry" / "harness-audit.ndjson"  # legacy v1
    if not path.exists():
        path = project_dir / "harness-audit.ndjson"  # legacy v0
    if not path.exists():
        return []
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    Useful when diagnosing a pause: shows declared state (.sprintfoundry/state/run-state.json) next
    ...
    if not run_state_path.exists():
        run_state_path = root / ".sprintfoundry" / "run-state.json"  # legacy v1
    if not run_state_path.exists():
        run_state_path = root / "run-state.json"  # legacy v0
    ...
    eval_paths = [
        ...
        *root.glob(".sprintfoundry/eval-results/eval-result-*.md"),  # legacy v1
        *root.glob("eval-result-*.md"),  # legacy v0
    ]
    ...
    print(f".sprintfoundry/state/run-state.json  mode={mode}  last_successful_sprint={declared_last}"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p_verify = sub.add_parser(
        "verify", help="Reconcile .sprintfoundry/state/run-state.json against eval-result verdicts."
    )
