...
| `harness-audit.ndjson` | Orchestrator + hooks | **Append-only forensic timeline** — never rewritten |
...
- Write reason to `run-state.json.last_failure_reason`
- Append short summary to `claude-progress.txt`
- Stop routing
...
| Human reviewed and chose action | Human (edits run-state.json) | `false` |
...
`run-state.json` tracks: `active_branch`, `base_branch`
...
- `state_transition` — every `run-state.json` change with `{key: [old, new]}` diffs
